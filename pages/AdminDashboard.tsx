...
const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'analytics' | 'inventory' | 'orders' | 'tickets' | 'customers' | 'offers' | 'home-settings'>('analytics');
  ...
  // Added missing state variables for category management
  ...
  const saveHomeSettings = () => {
    ...
    alert("Home settings updated successfully! 🏔️");
  };
  ...
  const handleSaveProduct = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isEditing) updateProduct(productData);
  ...
  const handleDuplicateProduct = (p: Product) => {
    ...
    const duplicatedProduct: Product = { ...p, id: newId, name: `${p.name} (Copy)`, status: p.stock > 0 ? 'in-stock' : 'out-of-stock' };
  ...
  const handleSaveOffer = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isEditingOffer) updateOffer(offerData);
    else addOffer(offerData);
  ...
  const handleReplyTicket = (e: React.FormEvent) => {
    ...
    addReply(selectedTicket.id, { authorId: 'admin-1', authorName: 'Admin', authorRole: 'admin', message: replyText });
    setReplyText('');
  };

  // Fixed missing state dependencies for category management
  const handleAddCategory = (e: React.FormEvent) => {
    e.preventDefault();
    if (newCategoryName.trim()) { addCategory(newCategoryName); setNewCategoryName(''); }
  };

  const handleStartEditCategory = (cat: string) => { setEditingCategory(cat); setEditCategoryValue(cat); };
  const handleUpdateCategory = () => { if (editingCategory && editCategoryValue.trim()) { updateCategory(editingCategory, editCategoryValue); setEditingCategory(null); setEditCategoryValue(''); } };
  const handleDeleteCategory = (cat: string) => { if (window.confirm(`Delete "${cat}"?`)) deleteCategory(cat); };
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        {activeTab === 'analytics' && (
          <div className="space-y-12 animate-fadeIn">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              ...
              <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100">
                <p className="text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-4">Returns</p>
                <h3 className="text-4xl font-bold serif text-red-400">{stats.returnedOrders}</h3>
              </div>
              <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100">
                <p className="text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-4">Avg Value</p>
          ...
          <div className="bg-white rounded-[3rem] shadow-xl border border-gray-100 overflow-hidden animate-fadeIn">
            <div className="p-10 flex justify-between items-center border-b border-gray-50">
               ...
               <div className="flex gap-4">
                  <button onClick={() => setShowCategoryModal(true)} className="bg-white border border-gray-200 text-[#4a5d4e] px-6 py-3 rounded-2xl text-[10px] font-bold uppercase">Categories</button>
                  ...
                  <tbody>
                    {products.map(p => (
                      <tr key={p.id} className="border-b border-gray-50">
                        ...
                        <td className="px-10 py-6 text-[10px] uppercase font-bold">{p.stock} Units</td>
                        <td className="px-10 py-6 text-right space-x-2">
                           <button onClick={() => handleDuplicateProduct(p)} className="p-2 text-purple-500"><ICONS.Copy /></button>
                           <button onClick={() => { setIsEditing(p); setMainImageBase64(p.image); setGalleryImagesBase64(p.images || []); setShowProductModal(true); }} className="p-2 text-blue-500"><ICONS.Edit /></button>
        ...
        {activeTab === 'home-settings' && (
          <div className="bg-white rounded-[4rem] p-12 shadow-xl border border-gray-100 animate-fadeIn">
            <div className="mb-10">
               <h2 className="text-3xl font-bold serif mb-2">Hero Section Management</h2>
               <p className="text-gray-400 text-sm">Update home page hero backgrounds. Multiple images will activate the slider effect.</p>
            </div>
            <div className="space-y-8">
               ...
               <div className="pt-8 border-t border-gray-50 flex justify-end">
                  <button onClick={saveHomeSettings} className="bg-[#4a5d4e] text-white px-10 py-4 rounded-2xl font-bold uppercase tracking-widest text-[11px] shadow-xl hover:-translate-y-1 transition-all">Save Changes</button>
               </div>
            </div>
          </div>
        )}

        {/* Existing logic for Orders, Tickets, etc. remains untouched based on provided source */}
      </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 backdrop-blur-xl bg-[#1a2323]/40 overflow-y-auto">
          <div className="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden animate-slideUp my-8">
            <div className="p-8 border-b border-gray-50 flex justify-between items-center bg-[#fcfbf7]">
              <h3 className="text-2xl font-bold serif">{isEditing ? 'Edit Batch' : 'New Harvest'}</h3>
            ...
            <form onSubmit={handleSaveProduct} className="p-10 space-y-8">
              <div className="grid md:grid-cols-2 gap-8">
                <input name="sku" defaultValue={isEditing?.id} placeholder="SKU" className="w-full bg-gray-50 rounded-xl p-4 text-sm" />
                <input name="name" defaultValue={isEditing?.name} required placeholder="Name" className="w-full bg-gray-50 rounded-xl p-4 text-sm" />
              </div>
              <textarea name="description" defaultValue={isEditing?.description} rows={3} placeholder="Description" className="w-full bg-gray-50 rounded-xl p-5 text-sm" />
              <div className="grid md:grid-cols-3 gap-6">
                <input type="number" name="price" defaultValue={isEditing?.price} required placeholder="Price" className="w-full bg-gray-50 rounded-xl p-4 text-sm" />
                <input type="number" name="stock" defaultValue={isEditing?.stock} required placeholder="Stock" className="w-full bg-gray-50 rounded-xl p-4 text-sm" />
                <input name="weight" defaultValue={isEditing?.weight} placeholder="Weight" className="w-full bg-gray-50 rounded-xl p-4 text-sm" />
              </div>
              <button type="submit" className="w-full bg-[#1a2323] text-white py-5 rounded-2xl font-bold uppercase tracking-widest text-[11px]">Save Changes</button>
            </form>
          </div>
        </div>
      )}
