...
const Header: React.FC = () => {
  ...
  const headerClasses = isScrolled 
    ...
    : 'bg-white/20 backdrop-blur-md py-5 border-b border-black/5';
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between gap-8">
          ...
          <nav className="hidden lg:flex items-center gap-10">
            {navLinks.map((link) => (
              <Link 
                key={link.path} 
                to={link.path}
                className={`text-[11px] uppercase tracking-[0.2em] font-bold hover:text-[#4a5d4e] transition-all relative group ${
          ...
          <div ref={searchRef} className="hidden md:flex flex-grow max-w-sm relative">
            <form onSubmit={handleSearch} className="w-full relative">
              <input 
                ...
                className={`w-full border-none rounded-xl py-2.5 pl-5 pr-12 text-xs outline-none transition-all duration-300 ${inputBg} text-gray-800 placeholder-gray-500`}
              />
            ...
            {isSearchFocused && searchQuery && (
              <div className="absolute top-full left-0 right-0 mt-3 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden animate-fadeIn text-gray-800">
                <div className="p-3">
                  <p className="px-3 py-2 text-[10px] uppercase font-bold text-gray-400 tracking-widest">Related Products</p>
                  {searchResults.length > 0 ? (
                    searchResults.map((product) => (
                      ...
                      >
                        ...
                        <div>
                          <p className="text-sm font-bold group-hover:text-[#4a5d4e]">{product.name}</p>
                          <p className="text-xs text-gray-400">₹{product.price}</p>
                        </div>
                      </Link>
                    ))
                  ) : (
                    <div className="p-6 text-center text-gray-400 text-xs italic">No matching products found.</div>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="flex items-center gap-6">
            ...
            <Link to="/cart" className={`relative transition-opacity hover:opacity-60 ${textClasses}`}>
              <ICONS.Cart />
              {totalItems > 0 && (
                <span className="absolute -top-2 -right-2 bg-[#8b5e3c] text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
                  {totalItems}
                </span>
              )}
            </Link>
            {user && (
              <button onClick={handleLogout} className={`transition-opacity hover:opacity-60 ${textClasses}`} title="Logout">
                <ICONS.Logout />
              </button>
            )}
          </div>
...
const Footer: React.FC = () => (
  <footer className="bg-[#1a2323] text-white pt-24 pb-12">
    <div className="max-w-7xl mx-auto px-6 lg:px-12">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">
        <div className="space-y-6">
          <h3 className="text-2xl font-bold serif">Vanphal Farms</h3>
        ...
        <div>
          ...
          <form className="relative" onSubmit={(e) => e.preventDefault()}>
            <input 
              ...
              className="w-full bg-white/5 border border-white/10 px-5 py-4 text-sm rounded-xl focus:outline-none focus:border-white/40 transition-colors"
            />
            <button className="absolute right-2 top-2 bottom-2 bg-white text-black px-4 rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-gray-200 transition-colors">
              Join
            </button>
          </form>
        </div>
